...
We start up our Virtuoso server, wait for it to come up, and then
connect:
...
consider the `nycflights13` data. First, we must represent any primary
or foreign keys in any table as URIs, indicated by a prefix, and not by
bare strings:
...
a prefix based on the table name.
...
Transform JSON (or list data) into triples. In this case, we have a
large JSON blob (or R list) containing metadata on all rOpenSci
packages:

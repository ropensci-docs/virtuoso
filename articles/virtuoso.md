...
We can control any `virtuoso` server started with
...
Advanced usage note:
[`vos_start()`](https://docs.ropensci.org/virtuoso/reference/vos_start.md)
invisibly returns a `processx` object which we can pass to other server
control functions, or access the embedded `processx` control methods
directly. The `virtuoso` package also caches this object in an
environment so that it can be accessed directly without having to keep
track of an object in the global environment. Use
[`vos_process()`](https://docs.ropensci.org/virtuoso/reference/vos_process.md)
to return the `processx` object. For example:
